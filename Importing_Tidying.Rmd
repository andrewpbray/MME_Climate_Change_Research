...
First we strip out unneeded columns and tidy.
...
**Question**: In your original code, you create a `zscore_peak_temp` variable that I don't
really understand. Could you tell me what you're going for there?

Turn each of the temperature readings into z-scores based on their deviation from 
that lake's monthly data.
...
main_data_no_spatial <- main_data_spread %>%
  ...
  summarise(MME = max(as.numeric(MME))-1, 
            Ice_Duration = mean(ice_duration_days, na.rm = TRUE), 
            Schmidt = mean(schmidt_daily_annual_sum, na.rm = TRUE), 
            Variance_After_Ice_30 = mean(coef_var_0.30, na.rm = TRUE), 
            Variance_After_Ice_60 = mean(coef_var_30.60, na.rm = TRUE), 
            Cumulative_Above_0 = mean(gdd_wtr_0c, na.rm = TRUE), 
            Cumulative_Above_5 = mean(gdd_wtr_5c, na.rm = TRUE), 
            Cumulative_Above_10 = mean(gdd_wtr_10c, na.rm = TRUE), 
            Mean_Surf_Temp = max(mean_surf, na.rm = TRUE), 
            Max_Surf_Temp = max(max_surf, na.rm = TRUE), 
            Mean_Bot_Temp = max(mean_bot, na.rm = TRUE), 
            Max_Bot_Temp = max(max_bot, na.rm = TRUE), 
            Mean_Surf_Zscore = max(mean_surfZ, na.rm = TRUE), 
            Max_Surf_Zscore = max(max_surfZ, na.rm = TRUE), 
            Mean_Bot_Zscore = max(mean_botZ, na.rm = TRUE), 
            Max_Bot_Zscore = max(max_botZ, na.rm = TRUE), 
            Summerkill = max(Summerkill, na.rm = TRUE), 
            Winterkill = max(winterkill, na.rm = TRUE), 
            Infection = max(Infection, na.rm = TRUE), 
            Anthropogenic = max(Anthropogenic, na.rm = TRUE)) %>%
  ungroup() 
```
