...
MME <- MME %>%
  ...
  dplyr::select(-contains('County'), 
                ...
                -contains('Cause.Desc'),
                -contains('Site.Seq.No'),
                -contains('Fishkill.Inv.Seq.No'),
                -contains('Location.QA.Comment'),
                -contains('Activity.Desc'),
                -contains('Recommended.Action.Desc'), 
                -contains('Fish.Kill.Comment'), 
                -contains('Live.Fish.Desc')) %>%
...
t3 <- thermal %>%
  ...
  dplyr::select(site_id, Year, Month, everything())

thermal$Month <- str_to_title((thermal$Month)) 
```

### Questions
1. It looks like the WBIC is NA for a good number of the lakes. Do we know if these
are zeros or missing? Are we treating them as zeros or missing? (I'm guessing we're
treating them as zeros, which seems like the best we can do.)
