...
MME <- left_join(MME, NHD_WBIC, by = "WBIC")
...
For the thermal data, we first only retain distinct `Year` X `site_id` combinations.
In general, these were distinct, but there were a few anomolous `site_id` that
had multiple records for each year.

```{r}
thermal <- thermal %>%
  distinct(Year, site_id, .keep_all = TRUE)
